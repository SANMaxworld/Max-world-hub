...
function displayItems(items, isInitialLoad = false) {
    ...
    if (isSeriesPage) filtered = items.filter(i => i.category === 'SERIES');
    else if (isMoviePage) filtered = items.filter(i => i.category === 'MOVIES');
    ...
    filtered.forEach(item => {
        ...
        div.innerHTML = `
            <img src="${item.logo}" class="${imgClass}">
...
function openAdvancedModal(id) {
    ...
    modal.innerHTML = `
        ...
        <div class="modal-content-full">
            <h2 style="color:#ff0000; font-size:1.5rem;">${item.name}</h2>
            <img src="${item.logo}" style="width:100%; border-radius:15px; border:1px solid #ff0000; margin:15px 0;">
            ...
            <h3 style="margin:20px 0 10px; border-left:4px solid #ff0000; padding-left:10px;">Downloads</h3>
            <div class="ep-list">
                ${item.episodes.map(e => `
                    <div class="ep-row">
                        <span>${e.ep}</span>
