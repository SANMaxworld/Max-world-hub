...
function displayItems(items, isInitialLoad = false) {
    ...
    filtered.forEach(item => {
        ...
        // --- STAGE: MOVIE SPECIAL LAYOUT ---
        // Isse Movies ka text center mein rahega aur card upar nahi bhagega
        if (item.category === 'MOVIES') {
            div.style.cssText = "display: flex; align-items: center; gap: 15px; padding: 12px; background: rgba(255,255,255,0.05); border-radius: 15px; margin-bottom: 15px; border: 1px solid rgba(255,255,255,0.1);";
        }
        ...
        div.innerHTML = `
            ...
            <div style="flex:1; display: flex; flex-direction: column; justify-content: center;">
                <h4 style="font-size:1rem; color:#fff; margin-bottom:2px; font-weight:600;">${item.name}</h4>
                
                <p class="${item.category === 'MOVIES' ? 'movie-desc' : ''}" style="${item.category !== 'MOVIES' ? 'font-size:0.75rem; color:#bbb;' : ''}">
                    ${item.desc}
                </p>
            ...
            <div style="margin-left: 8px;">
                ${item.isSeries 
                    ...
                    : `<a href="${item.url}" target="_blank" class="get-btn" style="text-decoration:none; display:inline-block;">GET</a>`}
            </div>
        `;
...
// 🍿 3. Series Detail Modal (No Changes here as per your request)
...
window.addEventListener('DOMContentLoaded', () => { if (isSeriesPage) displayItems(mwHubData, true); });
if (searchInput) { searchInput.addEventListener('input', () => displayItems(mwHubData)); }
window.onclick = (event) => { if (event.target == document.getElementById('seriesModal')) closeModal(); }

function closePopup() {
    const popup = document.getElementById('welcome-popup');
    if (popup) { popup.style.display = 'none'; sessionStorage.setItem('popupShown', 'true'); }
}
